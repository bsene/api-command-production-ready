...
(** [handle ~logger ~api_key ~catalog event] authenticates via [x-api-key],
    serves [GET /products] from [catalog], and otherwise echoes availability.
    Every branch returns an {!Event.response}; error bodies match Go's exact
    strings. *)
val handle :
  logger:Log.t ->
  api_key:string ->
